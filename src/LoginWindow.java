...
public class LoginWindow {
			...
			public void handle(ActionEvent e) {
				try {
					if(login(userId, password)) {
						Alert alert = new Alert(AlertType.INFORMATION, "Login Successfull!", ButtonType.CLOSE);
						alert.showAndWait();
						Main.loadWelcomeWindowD(userId.getText());						
					}
					else {
						Alert alert = new Alert(AlertType.INFORMATION, "Incorrect Username/Password!\nPlease Try again!", ButtonType.CLOSE);
						alert.showAndWait();						
					}
	...
	public boolean login(TextField userid, TextField pass) throws ClassNotFoundException, SQLException {
		EARSdatabase db = new EARSdatabase();
		ResultSet rs = db
				.queryDatabase("SELECT password " + "FROM security " + "WHERE userid = '" + userid.getText() + "';");
		rs.next();

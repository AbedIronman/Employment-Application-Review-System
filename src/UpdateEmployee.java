...
public class UpdateEmployee {
	...
	public VBox getLeft() {
		...
		update.setOnMouseEntered(new EventHandler<MouseEvent>() {
			@Override
		...
		update.setOnAction(e -> {
			updateDatabase(changes, value, empId);
		});
	...
	public void updateDatabase(ComboBox<String> data, TextField value, TextField empId) {
		// WRITE IT HERE
		try {
			db = new EARSdatabase();
			if (data.getValue() == "First Name") {
				db.updateDatabase("UPDATE employees " + "SET first_name ='" + value.getText() + "' WHERE employee_id= '"
						+ empId.getText() + "'");
			} else if (data.getValue() == "Last Name") {
				db.updateDatabase("UPDATE employees " + "SET last_name ='" + value.getText() + "' WHERE employee_id= '"
						+ empId.getText() + "'");
			} else if (data.getValue() == "Phone No.") {
				db.updateDatabase("UPDATE employees " + "SET phone ='" + value.getText() + "' WHERE employee_id= '"
						+ empId.getText() + "'");
			} else if (data.getValue() == "Age") {
				db.updateDatabase("UPDATE employees " + "SET age ='" + value.getText() + "' WHERE employee_id= '"
						+ empId.getText() + "'");
			} else if (data.getValue() == "Salary") {
				db.updateDatabase("UPDATE employees " + "SET salary_perhour ='" + value.getText()
						+ "' WHERE employee_id= '" + empId.getText() + "'");
			} else {

				Alert alert = new Alert(AlertType.INFORMATION, "Please select an option!", ButtonType.CLOSE);
				alert.showAndWait();
			}
			db.closeConnection();
			lv.setItems(getList());
	...
	public ObservableList<String> getList() throws ClassNotFoundException, SQLException {
		...
		rs = db.queryDatabase("SELECT * " + "FROM employees; ");

		for (int i = 0; i < array.length; i++) {
			rs.next();

			array[i] = String.format("%17s %25s %25s %25s %25s %25s", rs.getString(1), rs.getString(2), rs.getString(3),

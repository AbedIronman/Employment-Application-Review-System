...
public class DeleteApplicant {
	...
	public Scene getDeleteApplicant() throws ClassNotFoundException, SQLException {
	...
	public VBox getLeft() {
		...
		Text text = new Text("Delete Applicant");
	...
	public void updateDatabase(TextField userId) throws ClassNotFoundException, SQLException {
		...
		db.updateDatabase("DELETE FROM applicant WHERE applicant_id = '"+userId.getText()+"'; ");
		lv.setItems(getList());
	}
	...
	public VBox getRight() throws ClassNotFoundException, SQLException {
		...
		Text empid = new Text("Applicant ID");
		...
		review.setStyle("-fx-font-family: Arial; -fx-font-size: 15;");
		review.minWidth(vb.getMaxWidth()/6);
		review.setFill(Color.WHITE);
		
		attr.getChildren().addAll(empid, fname, lname, phone, age, review);
	...
	public ObservableList<String> getList() throws ClassNotFoundException, SQLException{
		...
		for(int i = 0; i<array.length; i++) {
			...
			array [i] = String.format("%15s %27s %27s %32s %15s %27s", rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(7));
